...
import { buildLongHistory } from "../examples/shared/fakeStation.mjs";
...
/* Fourteen months, fifteen-minute cadence, generated by the same fixture
 * every demo and every long-range gallery asset shares (examples/shared) —
 * a season's rose and a typical day are the same fake station, not two
 * hand-tuned coincidences. Seeded and clocked off NOW_MS, so it regenerates
 * byte-stable like everything else here. */
const LONG_HISTORY = buildLongHistory({ nowMs: NOW_MS, days: 420, periodMinutes: 15 });
const SEASON_ROSE_POINTS = core.filterByTimeOfDay(
  core.filterByMonth(LONG_HISTORY, core.METEOROLOGICAL_SEASON_MONTHS.summer),
  9 * 60,
  15 * 60,
);
...
/* The DailyPattern drawing, hand-rolled for the same reason chartSvg is:
 * the real component measures its width with a ResizeObserver, which never
 * fires under renderToStaticMarkup, so server rendering it directly would
 * draw nothing. Every slot becomes a synthetic point at its own midpoint —
 * exactly the technique station/react/components/DailyPattern.tsx uses —
 * so chartFrame/chartScales/averagePoints/thinVanes/vaneTicks all apply
 * unchanged. No lull-gust band: dailyPattern reports one vector-mean speed
 * per slot, never a spread. */
const DAILY_PATTERN_EPOCH_MS = Date.parse("2000-01-01T00:00:00Z");
...
function dailyPatternSvg({ points, slotMinutes, thresholds, width, x, y, idPrefix }) {
  const slots = core.dailyPattern(points, { slotMinutes });
  const synthetic = slots.map((slot) => ({
    observedAt: new Date(DAILY_PATTERN_EPOCH_MS + (slot.startMinuteOfDay + slotMinutes / 2) * 60_000).toISOString(),
  ...
  const frame = core.chartFrame(width);
  const scales = core.chartScales(synthetic, frame);
  const vanes = core.thinVanes(synthetic);
  const parts = [];
  ...
  const voidSlots = slots.filter((slot) => slot.sampleCount === 0);
  if (voidSlots.length > 0) {
    parts.push(
      `<defs><pattern id="${idPrefix}-hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line class="meteo-wind-gap-hatch" x1="0" y1="0" x2="0" y2="6"/></pattern></defs>`,
      ...voidSlots.map((slot) => {
        const startX = scales.xAtMs(DAILY_PATTERN_EPOCH_MS + slot.startMinuteOfDay * 60_000);
        const endX = scales.xAtMs(DAILY_PATTERN_EPOCH_MS + (slot.startMinuteOfDay + slotMinutes) * 60_000);
        return `<rect fill="url(#${idPrefix}-hatch)" x="${n(startX)}" y="${n(frame.plotTop)}" width="${n(endX - startX)}" height="${n(frame.plotBottom - frame.plotTop)}"/>`;
      }),
    );
  }
  for (let index = 1; index < synthetic.length; index += 1) {
    const [previous, point] = [synthetic[index - 1], synthetic[index]];
    const segmentBand = core.speedBand((previous.averageMps + point.averageMps) / 2, thresholds);
    parts.push(
      `<line class="meteo-wind-mean-segment meteo-band-${segmentBand}" x1="${n(scales.xAt(previous.observedAt))}" y1="${n(scales.yAt(previous.averageMps))}" x2="${n(scales.xAt(point.observedAt))}" y2="${n(scales.yAt(point.averageMps))}"/>`,
    );
  ...
  for (const tick of core.vaneTicks(vanes, scales)) {
    const anchor = tick.index === 0 ? "start" : tick.index === 4 ? "end" : "middle";
    parts.push(text(tick.x, frame.labelRow, "meteo-tick", formatMinuteOfDay((tick.timeMs - DAILY_PATTERN_EPOCH_MS) / 60_000), anchor));
  }
  return `<svg x="${n(x)}" y="${n(y)}" width="${frame.width}" height="${frame.height}" viewBox="0 0 ${frame.width} ${frame.height}">${parts.join("")}</svg>`;
}
...
function galleryRoseSeasonSvg() {
  const markup = renderToStaticMarkup(h(WindRose, { points: SEASON_ROSE_POINTS, thresholds: THRESHOLDS_DECLARED }));
  const calmPercent = Math.round(core.windRose(SEASON_ROSE_POINTS).calmFraction * 100);
  const body =
    `<rect class="hw-card" x="6.5" y="6.5" width="267" height="223" rx="12"/>` +
    text(20, 26, "hw-micro", "SUMMER · MIDDAY") +
    extractSvg(markup, "meteo-wind-rose-svg", 45, 20) +
    text(140, 226, "hw-dim", words.percentCalm(calmPercent), "middle");
  return svgDocument(280, 244, "light", "WindRose filtered to summer midday, 14 months of fake history", body);
}

function galleryDailyPatternSvg() {
  const slotMinutes = core.DAILY_PATTERN_DEFAULT_SLOT_MINUTES;
  const body =
    `<rect class="hw-card" x="6.5" y="6.5" width="267" height="171" rx="12"/>` +
    dailyPatternSvg({
      points: LONG_HISTORY,
  ...
  return svgDocument(280, 184, "light", "DailyPattern: a typical day across 14 months of fake history", body);
}
...
const ASSETS = {
  ...
  "gallery-daily-pattern.svg": galleryDailyPatternSvg(),
  "gallery-rose-season.svg": galleryRoseSeasonSvg(),
};
